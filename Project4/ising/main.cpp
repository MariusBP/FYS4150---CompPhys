...
void WriteResultstoFile(int, int, int, double, vec);
...
void MetropolisSampling(int NSpins, int MCcycles, double Temperature, vec &ExpectationValues)
{
  ...
  for (int cycles = 1; cycles <= MCcycles; cycles++){
    ...
    WriteResultstoFile(NSpins, MCcycles, count, Temperature, ExpectationValues);
  }
...
void WriteResultstoFile(int NSpins, int MCcycles, int count, double temperature, vec ExpectationValues)
{
  double norm = 1.0/((double) (MCcycles));  // divided by  number of cycles 
  double E_ExpectationValues = ExpectationValues(0);
  double E2_ExpectationValues = ExpectationValues(1);
  double M_ExpectationValues = ExpectationValues(2);
  double M2_ExpectationValues = ExpectationValues(3);
  double Mabs_ExpectationValues = ExpectationValues(4);
